  ...
  return (
    <footer className="bg-[#0F172A] text-white">
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {practiceAreas.map((area) => (
                <li key={area.slug}>
                  <Link href={`/practice-areas/${area.slug}`} className="text-white/50 hover:text-white text-sm transition-colors">
                    {area.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-3 text-sm text-white/30">
